package com.anhtester.Bai10_Annotations;

import org.testng.annotations.*;

public class ParentTestClass {
    @BeforeSuite
    public void beforeSuite() {
        System.out.println("PARENT: Đây là Before Suite 1");
    }

    @AfterSuite
    public void afterSuite() {
        System.out.println("PARENT: Đây là After Suite 1");
    }

    @BeforeTest
    public void beforeTest() {
        System.out.println("PARENT: BeforeTest 1: Chạy trước tất cả các test trong một thẻ <test>");
    }

    @AfterTest
    public void afterTest() {
        System.out.println("PARENT: AfterTest 1: Chạy sau tất cả các test trong một thẻ <test>");
    }

    @BeforeClass
    public void beforeClass() {
        System.out.println("PARENT: BeforeClass 1: Chạy trước tất cả các test method trong class này");
        System.out.println("PARENT: Thực hiện Login trước test case trong class này bằng email password");
    }

    @AfterClass
    public void afterClass() {
        System.out.println("PARENT: AfterClass 1: Chạy sau tất cả các test method trong class này");
    }

    @BeforeGroups("smoke")
    public void beforeGroup() {
        System.out.println("PARENT: BeforeGroup 1: Chạy trước nhóm smoke");
    }

    @BeforeMethod
    public void beforeMethod() {
        System.out.println("PARENT: BeforeMethod 1: Chạy trước mỗi phương thức test - Mở Browser");
    }

    @AfterMethod
    public void afterMethod() {
        System.out.println("PARENT: AfterMethod 1: Chạy sau mỗi phương thức test - Đóng Browser");
    }

    @Test(groups = "smoke")
    public void test_method_01() {
        System.out.println("PARENT: Đây là test method 01");
    }

    @Test
    public void test_method_02() {
        System.out.println("PARENT: Đây là test method 02");
    }

    @Test
    public void test_method_03() {
        System.out.println("PARENT: Đây là test method 03");
    }
}
